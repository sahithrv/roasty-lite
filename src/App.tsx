...
function App() {
  ...
  const reviewQueue = suggestedClips.filter(
    (clip) => clip.humanDecision === "Needs Review" || (clip.humanDecision === "Pending" && clip.recommendedAction === "Review"),
  );
...
function HumanReviewQueue({ clips, onDecision }: { clips: SuggestedClip[]; onDecision: (clipId: string, decision: HumanDecision) => void }) {
  return (
    <section className="panel review-panel">
      <PanelTitle title="Human Review Queue" subtitle="Creator is the final approver" />
      {clips.length === 0 ? (
        <EmptyState text="Review items will appear here when AI says Review or the creator flags a moment." />
      ) : (
        <div className="review-list">
          {clips.map((clip) => (
            <article className="review-item" key={clip.id}>
              <div>
                <span className={`moment-badge ${momentStyles[clip.momentType]}`}>{clip.momentType}</span>
                <h3>{clip.eventType}</h3>
                <p>{clip.finalCaption}</p>
              </div>
              <div className="button-row compact">
                <button onClick={() => onDecision(clip.id, "Saved")}>Approve</button>
                <button className="danger-button" onClick={() => onDecision(clip.id, "Discarded")}>
                  Reject
                </button>
              </div>
